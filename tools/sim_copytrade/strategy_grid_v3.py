"""Strategy grid v3: maxf=20 locked, full sweep, sort by PnL with ROI floor."""
...
def simulate(markets, agg_thr, agg_win_s, skip_n, copy_mode, copy_amt, copy_cap, max_follows):
    ...
    return {
        'mkts': mkts_followed, 'trades': total_trades,
        'copy_usd': round(total_copy, 2), 'pnl': round(total_pnl, 2),
...
def main():
    ...
    # Grid: maxf=20 locked
    thresholds = [200, 300, 500, 1000, 1500, 2000, 3000]
    windows = [15*60, 30*60, 60*60, 120*60]
    skips = [0, 3, 5, 8, 10, 12, 15]
    copy_cfgs = [
        ('fixed', 10, 10, 'fix$10'),
        ('fixed', 25, 25, 'fix$25'),
        ('fixed', 50, 50, 'fix$50'),
        ...
        ('prop', 0.005, 50, 'p0.5%c50'),
        ('prop', 0.005, 75, 'p0.5%c75'),
        ('prop', 0.005, 150, 'p0.5%c150'),
        ('prop', 0.01, 50, 'p1%c50'),
        ('prop', 0.01, 75, 'p1%c75'),
        ...
        ('prop', 0.10, 75, 'p10%c75'),
        ('prop', 0.10, 150, 'p10%c150'),
        ('prop', 0.15, 150, 'p15%c150'),
        ('prop', 0.20, 150, 'p20%c150'),
    ]
    MAX_FOLLOWS = 20

    total = len(thresholds) * len(windows) * len(skips) * len(copy_cfgs)
    print(f"Running {total:,} combos (maxf=20 locked)...")
    results = []
    done = 0
        ...
        for win in windows:
            for skip in skips:
                for mode, amt, cap, label in copy_cfgs:
                    r = simulate(markets, thr, win, skip, mode, amt, cap, MAX_FOLLOWS)
                    r['thr'] = thr
                    r['win'] = win // 60
                    r['skip'] = skip
                    r['copy'] = label
    ...
    by_roi = sorted([r for r in results if r['roi'] is not None and r['roi'] > 0],
                    key=lambda x: x['roi'], reverse=True)

    sweet = [r for r in results if r['roi'] is not None and r['roi'] >= 0.025 and r['pnl'] >= 2000]
    sweet.sort(key=lambda x: (x['pnl'], x['roi']), reverse=True)

    hdr = f"{'Thr':>5} {'Win':>4} {'Skip':>5} {'Copy':>12} {'Mkts':>5} {'Trades':>7} {'CopyUSD':>12} {'PnL':>10} {'ROI':>7} {'$/mkt':>7}"
    def pr(lst, n=40):
        print(hdr)
        print("-" * len(hdr))
        for r in lst[:n]:
            roi_s = f"{r['roi']*100:+.1f}%"
            per_mkt = r['pnl'] / r['mkts'] if r['mkts'] > 0 else 0
            print(f"${r['thr']:>4} {r['win']:>3}m {r['skip']:>5} {r['copy']:>12} {r['mkts']:>5} {r['trades']:>7} ${r['copy_usd']:>11,.0f} ${r['pnl']:>+9,.0f} {roi_s:>7} ${per_mkt:>+6.0f}")

    print(f"\n{'='*100}")
    print(f"TOP 40 BY PNL (ROI >= 2%, maxf=20)")
    pr(viable)

    print(f"\nTOP 30 BY ROI (maxf=20)")
    pr(by_roi, 30)

    print(f"\nSWEET SPOT (ROI >= 2.5% AND PnL >= $2,000, maxf=20)")
    pr(sweet)

    ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    out = SIM_ROOT / 'output' / f'strategy_grid_v3_maxf20_{ts}.json'
    payload = {
        'generated_at': datetime.now(timezone.utc).isoformat(),
        'leader_pnl': round(leader_pnl, 2),
        'max_follows': MAX_FOLLOWS,
        'total_combos': total,
        'top40_by_pnl': viable[:40],
