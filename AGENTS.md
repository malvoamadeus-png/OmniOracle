...
- 工具代码不得 import `backend.*`、生产 `copytrade.*`、根目录业务脚本，或通过 `sys.path` 指向 PolySport 项目根目录。
- 工具不得直接读取生产跟单 SQLite、账户 TOML、生产 `.env`、Supabase 主表或根目录 `metrics_fresh.sqlite`。
- 工具需要的公共 API helper、指标口径或配置快照，应复制/封装在工具目录内部，并在该工具 README 写清楚来源和差异。
- 工具运行数据只写入自己的 `runtime/`、`output/`、`.cache/` 等目录。
- 如果发现某个工具必须复用后端逻辑，优先复制稳定纯函数或抽成本工具内部模块；不要为了工具方便让后端承担研究代码依赖。
- `tools/legacy_address_metrics/` 是历史兼容例外，少数旧脚本会读取 backend copytrade SQLite/accounts；不要把这个例外复制到新工具。
...
- `reconcile_redeemed_positions` must be non-destructive:
...
- Add `repair_phantom_positions` before daily rebuild:
